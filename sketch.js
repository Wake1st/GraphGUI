...
function mousePressed() {
  ...
  if (hitEdge) {
    ...
    let vertEdges = getEdges(hitVertex);
    focusOnSelection(hitVertex, vertEdges);
  }
}
...
function mouseReleased() {
  if (isDrawing) {
    ...
    if (hitVertex !== undefined && hitVertex !== headVertexDrawing) {
      let e = {head: headVertexDrawing, tail: hitVertex};
      edges.push(e);
...
function drawItem(buffer, item, i) {
  ...
  buffer.rect(x, y, itemWidth, itemHeight * item.s, itemNodeR);
...
function getEdges(v) {
  return edges.filter(e => e.head === v || e.tail === v);
}
